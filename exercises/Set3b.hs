...
sums i = go 1 1
  where
    go currentIndex accumulatedSum
      | currentIndex > i = []
      | otherwise = accumulatedSum : go (currentIndex + 1) (accumulatedSum + currentIndex + 1)
...
indexDefault xs i def
  | i < 0 = def
  | otherwise = indexDefault' xs i def
  where
...
sumsOf (x : xs) = go x xs
  where
    go currentSum [] = [currentSum]
    go currentSum (y : ys) = currentSum : go (currentSum + y) ys
