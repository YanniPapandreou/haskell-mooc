...
longest xss = go [] xss
  where
    go best [] = best
    go best (xs : rest)
      | length xs < length best = go best rest
      | length xs > length best || head xs < head best = go xs rest
...
incrementKey _ [] = []
incrementKey k ((key, val) : rest) =
  if key == k
    then (key, val + 1) : incrementKey k rest
    else (key, val) : incrementKey k rest
...
freqs xs = foldr updateFreqs Map.empty xs
  where
    updateFreqs x counts = Map.alter helper x counts
    helper x = case x of
      Just val -> Just (val + 1)
      Nothing -> Just 1
...
transfer from to amount bank =
  if Map.member from bank && Map.member to bank && amount > 0
    then if fromHasEnough then adjustAccount to amount (adjustAccount from (-amount) bank) else bank
    else bank
  where
    fromHasEnough = Map.findWithDefault 0 from bank >= amount
    adjustAccount name delta b = Map.adjust (\x -> x + delta) name b
...
maxIndex = fst . last . sortBy (comparing snd) . assocs
