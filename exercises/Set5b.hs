...
cull _ Empty = Empty
cull val (Node v l r)
  | val /= v = Node v (cull val l) (cull val r)
  | otherwise = Empty
...
walk (StepL : rest) (Node _ l _) = walk rest l
walk (StepR : rest) (Node _ _ r) = walk rest r
...
set (StepL : rest) val (Node v l r) = Node v (set rest val l) r
set (StepR : rest) val (Node v l r) = Node v l (set rest val r)
...
search val tree = go [] tree
  where
    go _ Empty = Nothing
    go path (Node v l r)
      | v == val = Just path
      | otherwise = case go (path ++ [StepL]) l of
          Nothing -> go (path ++ [StepR]) r
          res -> res
