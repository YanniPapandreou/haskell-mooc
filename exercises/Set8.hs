...
blendColor (Color r1 g1 b1) (Color r2 g2 b2) = Color r g b
  where
    [r, g, b] = zipWith avg [r1, g1, b1] [r2, g2, b2]
    avg x y = (x + y) `div` 2

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine f (Picture f1) (Picture f2) = Picture (\coord -> f (f1 coord) (f2 coord))
...
rectangle x0 y0 w h = Shape f
  where
    f (Coord x y) = x >= x0 && y >= y0 && x < x0 + w && y < y0 + h
...
paintSolid color (Shape f) (Picture g) = Picture g'
  where
    g' coord
      | f coord = color
      | otherwise = g coord
...
paint (Picture fPat) (Shape fShape) (Picture fBase) = Picture f
  where
    f coord
      | fShape coord = fPat coord
      | otherwise = fBase coord
...
getNeighborCoords (Coord x y) =
  [Coord x y, Coord (x - 1) y, Coord (x + 1) y, Coord x (y - 1), Coord x (y + 1)]

getNeighborColors :: Picture -> Coord -> [Color]
getNeighborColors (Picture f) coord = map f (getNeighborCoords coord)

getAvgColor :: [Color] -> Color
getAvgColor cs = Color avgR avgG avgB
  where
    (sumR, sumG, sumB) = foldr (\(Color r g b) (sr, sg, sb) -> (sr + r, sg + g, sb + b)) (0, 0, 0) cs
    n = length cs
    avgR = sumR `div` n
    avgG = sumG `div` n
    avgB = sumB `div` n
...
instance Transform Blur where
  apply Blur p = Picture g
    where
      g coord = getAvgColor (getNeighborColors p coord)
